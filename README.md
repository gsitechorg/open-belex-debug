...
* [`src/belex_debug/styles.cljs`](src/belex_debug/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/belex_debug/`](src/belex_debug/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/belex_debug/core.cljs): contains the SPA entry point, `init`
* [`test/belex_debug/`](test/belex_debug/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `belex-debug` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/belex_debug/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns belex-debug.example
  (:require [belex-debug.config :as config])
